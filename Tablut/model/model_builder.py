...
def train():
    ...
    dataset_path = "./data/piedino-vs-becchi.txt"
    ...
    print(f"Train: {mean_squared_error(Ytrain, y_true)}")

    y_true = model.predict(Xtest)
    print(f"Train: {mean_squared_error(Ytest, y_true)}")

    with open("regressor.sav", "wb") as f:
          pickle.dump(model,f)
